...
# ✅ '지역' 컬럼 자동 지정 (ex: 행정구역, 행정기관 등)
def rename_region_column(df):
    possible_cols = ["지역", "행정구역", "시군구", "행정기관", "시도"]
    for col in df.columns:
        if any(keyword in col for keyword in possible_cols):
            df = df.rename(columns={col: "지역"})
            return df
    raise ValueError("❌ '지역' 또는 '행정구역' 컬럼을 찾을 수 없습니다.")
...
if uploaded_files and len(uploaded_files) == 2:
    ...
    for i, file in enumerate(uploaded_files):
        ...
        df = rename_region_column(df)             # ✅ '지역' 컬럼 설정
        df['연도'] = 2010 if i == 0 else 2025
        df["지역"] = df["지역"].apply(simplify_region_name)  # ✅ 지역명 간소화
        dfs.append(df)
    ...
    st.warning("⚠️ 반드시 **2010년과 2025년 CSV 파일 2개**를 업로드해주세요.")
